...
impl MailInfo {
    pub fn from_bytes(bytes: Bytes) -> Result<(Bytes, MailInfo), anyhow::Error> {
        ...
        if from.len() != 1 {
            return Err(anyhow::anyhow!("Found {} addresses in From", from.len()));
        }
        let from = from[0]
            .address()
            .map(|s| s.to_string())
        ...
        if to.len() != 1 {
            return Err(anyhow::anyhow!("Found {} addresses in To", to.len()));
        }
        let to = to[0]
            .address()
            .map(|s| s.to_string())
        ...
        let date = mail.date().map(|d| d.to_timestamp());
        Ok((
            body,
            Self {
                subject,
    ...
    pub fn into_file(self, body: Bytes) -> Result<Bytes, anyhow::Error> {
        ...
        let mut buf = Vec::with_capacity(body.len().div_ceil(3) * 4 + 1024);
